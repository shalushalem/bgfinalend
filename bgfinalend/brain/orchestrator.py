...
class AhviOrchestrator:
    ...
    def _styling_response(
        ...
        *,
        ...
        # --- STYLING FALLBACK WITH MINIMUM 3 SUGGESTIONS ---
        def _is_match(doc: Dict[str, Any], keywords: list[str]) -> bool:
        ...
        def _doc_id(doc: Dict[str, Any]) -> str:
            return str(doc.get("$id") or doc.get("id") or "").strip()

        def _build_bootstrap_outfits(docs: list[Dict[str, Any]], limit: int = 3) -> list[Dict[str, Any]]:
            tops = [d for d in docs if _is_match(d, ["top", "shirt", "blouse", "tee", "jacket", "hoodie"])]
            bottoms = [d for d in docs if _is_match(d, ["bottom", "pant", "trouser", "jean", "skirt", "short"])]
            shoes = [d for d in docs if _is_match(d, ["shoe", "sneaker", "boot", "heel", "sandal", "footwear"])]
            if not tops or not bottoms:
                return []

            generated: list[Dict[str, Any]] = []
            seen_combo_ids: set[str] = set()
            top_cap = min(3, len(tops))
            bottom_cap = min(3, len(bottoms))
            shoe_cap = min(3, len(shoes))

            for ti in range(top_cap):
                for bi in range(bottom_cap):
                    top = tops[ti]
                    bottom = bottoms[bi]
                    shoe = shoes[(ti + bi) % shoe_cap] if shoe_cap > 0 else {}
                    combo_id = f"fallback_{_doc_id(top)}_{_doc_id(bottom)}_{_doc_id(shoe)}"
                    ...
                    generated.append(
                        {
                            ...
                            "bottom": bottom,
                            "shoes": shoe,
                            "score": 8.5 - (0.1 * len(generated)),
                            "rank_score": 8.5 - (0.1 * len(generated)),
                        }
                    )
        ...
        wardrobe_source = context.get("wardrobe") or enriched_context.get("wardrobe")
        if wardrobe_source:
            safe_docs = self._normalize_documents(wardrobe_source)
            if not outfits:
                outfits = _build_bootstrap_outfits(safe_docs, limit=3)
            elif len(outfits) < 3:
                existing = {str(o.get("combo_id", "")).strip() for o in outfits if isinstance(o, dict)}
                extra = _build_bootstrap_outfits(safe_docs, limit=3)
                for row in extra:
                    combo = str(row.get("combo_id", "")).strip()
                    if combo and combo not in existing:
                        outfits.append(row)
                        existing.add(combo)
                    if len(outfits) >= 3:
                        break
        ...
        should_persist_style_boards = self._should_persist_style_boards(text=text, context=context)
        if fallback_depth > 0:
            logger.info(
            ...
            saved_outfit_ids = []
        elif not should_persist_style_boards:
            logger.info(
                "orchestrator.skip_persist_no_save_intent request_id=%s outfits=%s",
                request_id,
                len(outfits),
            )
            saved_outfit_ids = []
        else:
            saved_outfit_ids = self._persist_style_boards(
                appwrite=appwrite,
                user_id=user_id,
                outfits=outfits,
                request_id=request_id,
        ...
        for card in cards:
            ...
            value = str(
                card.get("board_id")
                or card.get("saved_board_id")
                or card.get("outfit_id")
                or ""
            ).strip()
            if value and value not in board_ids:
                board_ids.append(value)
        if not board_ids and saved_outfit_ids:
            board_ids = [str(x).strip() for x in saved_outfit_ids if str(x).strip()]
    ...
    def _should_persist_style_boards(self, *, text: str, context: Dict[str, Any]) -> bool:
        if bool(context.get("save_style_board")):
            return True
        ...
        return any(target in lowered for target in save_targets)

    def _persist_style_boards(
        self,
        *,
        appwrite: AppwriteProxy,
        user_id: str,
        ...
        saved_board_ids: list[str] = []
        context = context or {}
        try:
            existing_boards = self._normalize_documents(appwrite.list_documents("saved_boards", user_id=user_id))
            existing_hashes = {
                _hash_outfit(
                    {
                        "imageUrl": b.get("imageUrl") or b.get("image_url"),
                        "itemIds": b.get("itemIds") or b.get("item_ids") or [],
            ...
            logger.warning("orchestrator.saved_boards_load_failed request_id=%s error=%s", request_id, e)
            existing_hashes = set()
        ...
        def _occasion_for_board() -> str:
            slots = context.get("slots") if isinstance(context.get("slots"), dict) else {}
            direct = _first_text(
                slots.get("occasion") if isinstance(slots, dict) else "",
                context.get("occasion"),
            )
            if direct:
                return direct.strip().title()
            lowered = str(text or "").lower()
            if "party" in lowered:
                return "Party"
            if "office" in lowered or "work" in lowered:
                return "Office"
            if "vacation" in lowered or "trip" in lowered or "travel" in lowered:
                return "Vacation"
            return "Occasion"

        def _outfit_to_document(outfit: Dict[str, Any], outfit_hash: str) -> Dict[str, Any] | None:
            top = outfit.get("top") if isinstance(outfit.get("top"), dict) else {}
            bottom = outfit.get("bottom") if isinstance(outfit.get("bottom"), dict) else {}
            shoes = outfit.get("shoes") if isinstance(outfit.get("shoes"), dict) else {}
            pieces = [top, bottom, shoes, outfit]
            ...
            def pick(*keys: str) -> str:
                for piece in pieces:
                    if not isinstance(piece, dict):
                        continue
                    for key in keys:
                        val = _first_text(piece.get(key))
                        if val:
                            return val
                return ""

            image_url = pick("image_url", "imageUrl", "raw_image_url", "rawImageUrl")
            masked_url = pick("masked_url", "maskedUrl", "image_masked_url", "maskedImageUrl", "processed_image_url")
            ...
            for piece in (top, bottom, shoes):
                if not isinstance(piece, dict):
                    continue
                pid = _first_text(piece.get("$id"), piece.get("id"), piece.get("item_id"), piece.get("itemId"))
        ...
        for outfit in outfits:
            try:
                ...
                if outfit_hash in existing_hashes:
                    continue
                payload = _outfit_to_document(outfit, outfit_hash)
                if not payload:
                    logger.info(
                        "orchestrator.outfit_skip_saved_board request_id=%s reason=missing_image_urls hash=%s",
                        request_id,
                        outfit_hash,
                    )
                    continue
                ...
                saved_board_ids.append(doc.get("$id"))
                existing_hashes.add(outfit_hash)
            except Exception as e:
                logger.warning("orchestrator.saved_board_save_failed request_id=%s error=%s", request_id, e)
        return saved_board_ids
